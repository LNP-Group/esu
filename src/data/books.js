export const purpleBolt = {
    en: {
        title: "Purple Bolt",
        cover: "https://cdn.esufranchise.com/app/books/purple-bolt.png",
        synopsis: "An 18-year-old student gets superpowers after an experiment conducted by his father.",
        pageNumber: "TBA",
        releaseDate: "Late 2023",
        author: "Stelian Cioarec",
        premise: "Purple Bolt is the story of an 18-year-old student, living in Phebridge City, New York, who suffered an accident when he was five years old. He later learned that the accident changed his life and became a superhero known as Purple Bolt, inspiring little kids to do good and aim to be their best selves. But the road wasn’t always easy for him. He had faced hard decisions along the way, which cost him.",
        plot: "To be added.",
        phase: "Phase 1: The Ethereals"
    },
    ro: {
        title: "Fulgerul Violet",
        cover: "https://cdn.esufranchise.com/website/books/romanian/purple-bolt.png",
        synopsis: "Un student de 18 ani primește superputeri în urma unui experiment făcut de tatăl său.",
        pageNumber: "AFA",
        releaseDate: "Sfârșitul lui 2023",
        author: "Cioarec Stelian",
        premise: "Fulgerul Violet este povestea unui student în vârstă de 18 de ani, care trăiește în Phebridge City, New York, care a suferit un accident când avea cinci ani. Mai târziu, el a aflat că accidentul i-a schimbat viața și a devenit un super-erou cunoscut sub numele de Fulgerul Violet, inspirându-i pe copii să facă bine și să-și propună să fie cei mai buni indivizi. Dar drumul nu a fost întotdeauna ușor pentru el. S-a confruntat cu decizii grele pe parcurs, ceea ce l-a costat.",
        plot: "A fi adăugat.",
        phase: "Faza I: Etericii"
    },
    nl: {
        title: "Purple Bolt",
        cover: "https://cdn.esufranchise.com/website/books/english/purple-bolt.png",
        synopsis: "Een 18 jarige student krijgt superkrachten door een experiment uitgevoerd door zijn vader.",
        pageNumber: "N.t.b",
        releaseDate: "Eind 2023",
        author: "Stelian Cioarec",
        premise: "Purple Bolt is het verhaal van een 18 jarige student die woont in Phebridge City, New York. Toen hij vijf jaar oud was leed hij onder een ongeluk. Door het ongeluk was zijn leven veranderd en toen hij later ontdekte dat hij door het ongeluk superkrachten kreeg werd hij de superheld Purple Bolt die kleine kinderen inspireert om het goede te doen en om hun beste zelf te zijn. Maar de weg was niet altijd gemakkelijk voor hem, onderweg stond hij voor moeilijke beslissingen die hem duur kwamen te staan.",
        plot: "Wordt nader nog toegevoegd.",
        phase: "Fase 1: The Ethereals"
    },
    sv: {
        title: "Purple Bolt",
        cover: "https://cdn.esufranchise.com/website/books/english/purple-bolt.png",
        synopsis: "En artonårig student som får superkrafter efter en experiment utfördes av hans fader.",
        pageNumber: "S.l.t",
        releaseDate: "Sent 2023",
        author: "Stelian Cioarec",
        premise: "Purple Bolt är en berättelse av en artonårig student, som bor i Phebridge City, New York, som var med i en olycka när han var fem år gammal. Han får senare reda på att olyckan förändrades hans liv och blev känd som superhjälten Purple Bolt, och inspirerar små barn att göra goda handlingar och att dem ska bli den bästa versionen av dem själva. Men superhjälte vägen var inte alltid lätt för honom. Han har varit tvungen att ta svåra beslut som har som kostade honom.",
        plot: "Ska läggas till.",
        phase: "Fas 1: The Ethereals"
    }
}

export const thor = {
    en: {
        title: "Thor",
        cover: "https://cdn.esufranchise.com/app/books/thor.png",
        synopsis: "Thor, our Norse god of thunder has to deal with some problems that take place on Earth.",
        pageNumber: "TBA",
        releaseDate: "TBA",
        author: "Stelian Cioarec",
        premise: "To be added.",
        plot: "To be added.",
        phase: "Phase 1: The Ethereals"
    },
    ro: {
        title: "Thor",
        cover: "https://cdn.esufranchise.com/website/books/romanian/thor.png",
        synopsis: "Thor, zeul nordic al tunetelor, trebuie să se ocupe de niște probleme de pe Pământ.",
        pageNumber: "AFA",
        releaseDate: "AFA",
        author: "Cioarec Stelian",
        premise: "A fi adăugat.",
        plot: "A fi adăugat.",
        phase: "Faza I: Etericii"
    },
    nl: {
        title: "Thor",
        cover: "https://cdn.esufranchise.com/website/books/english/thor.png",
        synopsis: "Thor, onze Noorse God van de Donder, is ingewikkeld in wat problemen op aarde.",
        pageNumber: "N.t.b",
        releaseDate: "N.t.b",
        author: "Stelian Cioarec",
        premise: "Wordt nader nog toegevoegd.",
        plot: "Wordt nader nog toegevoegd.",
        phase: "Fase 1: The Ethereals"
    },
    sv: {
        title: "Tor",
        cover: "https://cdn.esufranchise.com/website/books/english/thor.png",
        synopsis: "Tor, vår nordiska åskgud måste ta itu med några problem på jorden.",
        pageNumber: "S.l.t",
        releaseDate: "S.l.t",
        author: "Stelian Cioarec",
        premise: "Ska läggas till.",
        plot: "Ska läggas till.",
        phase: "Fas 1: The Ethereals"
    }
}

export const madameWilhelmina = {
    en: {
        title: "Madame Wilhelmina",
        cover: "https://cdn.esufranchise.com/app/books/madame-wilhelmina.png",
        synopsis: "A witch has to deal with the extinction of her kind.",
        pageNumber: "TBA",
        releaseDate: "TBA",
        author: "Stelian Cioarec",
        premise: "After her town was reported for witchcraft, Sarah Allen is forced to witness the death of her kind, losing her parents and sister in the process. She has to deal with her loss and try and survive a world where she's not accepted for who she truly is.",
        plot: "To be added.",
        phase: "Phase 1: The Ethereals"
    },
    ro: {
        title: "Madame Wilhelmina",
        cover: "https://cdn.esufranchise.com/website/books/romanian/madame-wilhelmina.png",
        synopsis: "O vrăjitoare trebuie să treacă peste pieirea speciei sale.",
        pageNumber: "AFA",
        releaseDate: "AFA",
        author: "Cioarec Stelian",
        premise: "A fi adăugat.",
        plot: "A fi adăugat.",
        phase: "Faza I: Etericii"
    },
    nl: {
        title: "Madame Wilhelmina",
        cover: "https://cdn.esufranchise.com/website/books/english/madame-wilhelmina.png",
        synopsis: "Het verhaal van de laatste overlevende heks.",
        pageNumber: "N.t.b",
        releaseDate: "N.t.b",
        author: "Stelian Cioarec",
        premise: "Wordt nader nog toegevoegd.",
        plot: "Wordt nader nog toegevoegd.",
        phase: "Fase 1: The Ethereals"
    },
    sv: {
        title: "Madame Wilhelmina",
        cover: "https://cdn.esufranchise.com/website/books/english/madame-wilhelmina.png",
        synopsis: "En häxa måste ta itu med utrotning av hennes slag.",
        pageNumber: "S.l.t",
        releaseDate: "S.l.t",
        author: "Stelian Cioarec",
        premise: "Ska läggas till.",
        plot: "Ska läggas till.",
        phase: "Fas 1: The Ethereals"
    }
}

export const doctorHex = {
    en: {
        title: "Doctor Hex",
        cover: "https://cdn.esufranchise.com/app/books/doctor-hex.png",
        synopsis: "An archaeologist is doomed with magic powers after touching a sacred artifact.",
        pageNumber: "TBA",
        releaseDate: "TBA",
        author: "Stelian Cioarec",
        premise: "Arthur Williams, a famous British archaeologist, is sent by the Queen on a mission to verify the claims of a British soldier about Alexander the Great's tomb being found in a buried pyramid in Egypt. Upon arrival, he quickly discovers things aren't as they seem, making him question the nature of reality and its origins.",
        plot: "To be added.",
        phase: "Phase 1: The Ethereals"
    },
    ro: {
        title: "Doctor Hex",
        cover: "https://cdn.esufranchise.com/website/books/romanian/doctor-hex.png",
        synopsis: "Un arheolog e blestemat cu puteri magice după ce a atins un artefact sacru.",
        pageNumber: "AFA",
        releaseDate: "AFA",
        author: "Cioarec Stelian",
        premise: "A fi adăugat.",
        plot: "A fi adăugat.",
        phase: "Faza I: Etericii"
    },
    nl: {
        title: "Doctor Hex",
        cover: "https://cdn.esufranchise.com/website/books/english/doctor-hex.png",
        synopsis: "Een archeoloog die gedoemd is door magie nadat hun een heilig artefact aanraakte.",
        pageNumber: "N.t.b",
        releaseDate: "N.t.b",
        author: "Stelian Cioarec",
        premise: "Wordt nader nog toegevoegd.",
        plot: "Wordt nader nog toegevoegd.",
        phase: "Fase 1: The Ethereals"
    },
    sv: {
        title: "Doktor Hex",
        cover: "https://cdn.esufranchise.com/website/books/english/doctor-hex.png",
        synopsis: "En arkeolog blir dömd med magiska krafter efter att han röde en helig artefakt.",
        pageNumber: "S.l.t",
        releaseDate: "S.l.t",
        author: "Stelian Cioarec",
        premise: "Ska läggas till.",
        plot: "Ska läggas till.",
        phase: "Fas 1: The Ethereals"
    }
}

export const theGhost = {
    en: {
        title: "The Ghost",
        cover: "https://cdn.esufranchise.com/app/books/the-ghost.png",
        synopsis: "A murderer has to fight criminals in the real world, after his death, as a punishment from Lucifer for his crimes.",
        pageNumber: "TBA",
        releaseDate: "TBA",
        author: "Stelian Cioarec",
        premise: "Tom Abner, sentenced to death after a series of murders, finds himself at the gates of Hell, escorted by a demon to Lucifer's palace. When he's met face-to-face with Lucifer, he's sent back to Earth as a human ghost to fight off and stop criminals and rogue demons, and send them to Hell.",
        plot: "To be added.",
        phase: "Phase 1: The Ethereals"
    },
    ro: {
        title: "Fantoma",
        cover: "https://cdn.esufranchise.com/website/books/romanian/the-ghost.png",
        synopsis: "Un criminal trebuie să lupte cu infractori în lumea reală, după moartea sa, ca pedeapsă pentru crimele sale de la Lucifer.",
        pageNumber: "AFA",
        releaseDate: "AFA",
        author: "Cioarec Stelian",
        premise: "A fi adăugat.",
        plot: "A fi adăugat.",
        phase: "Faza I: Etericii"
    },
    nl: {
        title: "The Ghost",
        cover: "https://cdn.esufranchise.com/website/books/english/the-ghost.png",
        synopsis: "Een moordenaar die van Lucifer tegen criminelen moet vechten in de sterfelijke wereld na zijn dood als straf voor zijn misdaden.",
        pageNumber: "N.t.b",
        releaseDate: "N.t.b",
        author: "Stelian Cioarec",
        premise: "Wordt nader nog toegevoegd.",
        plot: "Wordt nader nog toegevoegd.",
        phase: "Fase 1: The Ethereals"
    },
    sv: {
        title: "Spöket",
        cover: "https://cdn.esufranchise.com/website/books/english/the-ghost.png",
        synopsis: "En mördare måste slås mot kriminella i den rikiga världen, efter hans död, som en bestraffning från Lucifer för hans brott.",
        pageNumber: "S.l.t",
        releaseDate: "S.l.t",
        author: "Stelian Cioarec",
        premise: "Ska läggas till.",
        plot: "Ska läggas till.",
        phase: "Fas 1: The Ethereals"
    }
}

export const theEthereals = {
    en: {
        title: "The Ethereals",
        cover: "https://cdn.esufranchise.com/app/books/the-ethereals.png",
        synopsis: "Our heroes will team-up for the first time to fight an unknown threat that attacks the Earth.",
        pageNumber: "TBA",
        releaseDate: "TBA",
        author: "Stelian Cioarec",
        premise: "To be added.",
        plot: "To be added.",
        phase: "Phase 1: The Ethereals"
    },
    ro: {
        title: "Etericii",
        cover: "https://cdn.esufranchise.com/website/books/romanian/the-ethereals.png",
        synopsis: "Eroii noștrii se unesc pentru prima dată pentru a lupta o amenințare necunoscută ce atacă Pământul.",
        pageNumber: "AFA",
        releaseDate: "AFA",
        author: "Cioarec Stelian",
        premise: "A fi adăugat.",
        plot: "A fi adăugat.",
        phase: "Faza I: Etericii"
    },
    nl: {
        title: "The Ethereals",
        cover: "https://cdn.esufranchise.com/website/books/english/the-ethereals.png",
        synopsis: "Onze helden zullen samenwerken voor de eerste keer om een onbekende dreiging tegen de aarde te bestrijden.",
        pageNumber: "N.t.b",
        releaseDate: "N.t.b",
        author: "Stelian Cioarec",
        premise: "Wordt nader nog toegevoegd.",
        plot: "Wordt nader nog toegevoegd.",
        phase: "Fase 1: The Ethereals"
    },
    sv: {
        title: "The Ethereals",
        cover: "https://cdn.esufranchise.com/website/books/english/the-ethereals.png",
        synopsis: "Våra superhjältar måste jobba tillsammans för första gången för att slås mot en okänt hot som attakerar jorden.",
        pageNumber: "S.l.t",
        releaseDate: "S.l.t",
        author: "Stelian Cioarec",
        premise: "Ska läggas till.",
        plot: "Ska läggas till.",
        phase: "Fas 1: The Ethereals"
    }
}

export const dracula = {
    en: {
        title: "Dracula: The First Vampire",
        cover: "https://cdn.esufranchise.com/website/books/english/dracula.png",
        synopsis: "A man becomes a vampire after being cursed by witch.",
        pageNumber: "TBA",
        releaseDate: "TBA",
        author: "Stelian Cioarec",
        premise: "In the year of 1623, in Transylvania, a peasant, slave to a noble, wants to change his life by becoming rich and to do so he asks for a witch's help. After his wish is fulfilled, he failed to pay the witch back and his soul is cursed to never rest. The curse turns him into a vampire, and therefore the first monster in the world.",
        plot: "To be added.",
        phase: "Phase 2: Rise of the Titans"
    },
    ro: {
        title: "Dracula: Primul vampir",
        cover: "https://cdn.esufranchise.com/website/books/romanian/dracula.png",
        synopsis: "Un om devine un vampir după ce este blestemat de o vrăjitoare.",
        pageNumber: "AFA",
        releaseDate: "AFA",
        author: "Cioarec Stelian",
        premise: "În Transilvania anului 1623, un țăran, sclav al unui nobil, voia să-și schimbe viața devenind bogat, iar pentru asta a cerut ajutorul unei vrăjitoare. După ce dorința sa îi este îndeplinită, nu a răsplătit vrăjitoarea și sufletul său e blestemat să nu se odihnească. Blestemul îl transformă în vampir și primul monstru din lume.",
        plot: "A fi adăugat.",
        phase: "Faza a II-a: Ascensiunea titanilor"
    },
    nl: {
        title: "Dracula: The First Vampire",
        cover: "https://cdn.esufranchise.com/website/books/english/dracula.png",
        synopsis: "Door de vervloeking van een heks is een man in een vampier veranderd.",
        pageNumber: "N.t.b",
        releaseDate: "N.t.b",
        author: "Stelian Cioarec",
        premise: "In het jaar 1623, Transsylvanië, een tot slaaf gemaakte boer van een edele wil zijn leven verbeteren door rijk te worden en vraagt een heks om hulp. Echter, nadat de heks zijn wens had vervuld had hij de heks niet betaald voor haar hulp en dus was zijn ziel vervloekt om te dwalen en nooit rust te vinden. Door de vervloeking veranderde hij in een vampier, en zodoende het eerste monster in de wereld.",
        plot: "Wordt nader nog toegevoegd.",
        phase: "Fase II: Rise of the Titans"
    },
    sv: {
        title: "Dracula: Första Vampyren",
        cover: "https://cdn.esufranchise.com/website/books/english/dracula.png",
        synopsis: "En man förvandlas till en vampyr efter att han blev förbannad av en häxa.",
        pageNumber: "S.l.t",
        releaseDate: "S.l.t",
        author: "Stelian Cioarec",
        premise: "År 1623, i Transylvania, ville en bonde, slav till en ädelsman, förändra sit liv genom att bli rik och för att göra det frågade han en häxa om hjälp. Efter att hans önskan gick i uppfyllelse, misslyckades han med att betala tillbaka till häxan och hans själ blev dömd till att aldrig vila. Förbannelsen förvandlar honom till en vampyr, och därefter det första monstret i världen.",
        plot: "Skall läggas till.",
        phase: "Fas II: Rise of the Titans"
    }
}